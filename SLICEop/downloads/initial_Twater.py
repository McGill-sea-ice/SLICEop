...
longueuil_in = pd.read_csv("/storage/jrieck/SLICEop/downloads/Twater/Tw_Longueuil_updated.csv").to_xarray().set_coords("Date").isel(index=slice(None, -1))
...
replace = [np.nan if tmp[i] == 'Bad' else float(tmp[i]) for i in range(0, len(tmp))]
longueuil_in["T"] = ("Date", replace)
# make time axis monotonically increasing and create daily averages
longueuil_in = longueuil_in.drop_vars("index").sortby("Date").resample(Date="1D").mean("Date")
# we remove outliers and compute a climatological seasonal cycle
# outliers are defined as "temperature changes by more than 0.1 degC per hour" and/or temperature > 30 degC
longueuil_smooth = longueuil_in.copy()
dTdt_ok = np.abs(longueuil_in["T"].ffill(dim="Date").differentiate("Date", datetime_unit="h")) < 0.1
longueuil_smooth["T"] = longueuil_in.T.where(dTdt_ok & (longueuil_in.T < 30.), other=np.nan)
...
extension = xr.Dataset(coords={"Date": pd.date_range(start="2022-01-01", end="2023-12-31", freq="1d")})
twoclim = np.hstack([longueuil_clim.T.values[0:-1], longueuil_clim.T.values[0:-1]])
...
thermistor = pd.read_csv("/storage/jrieck/SLICEop/downloads/Twater/StationLongueuil.dat", sep=",", header=3, low_memory=False).to_xarray()
...
thermistor = thermistor.drop_vars("index").sortby("Date").sel(Date=slice("2023-11-15", "2024-12-16"))
thermistor_daily = thermistor.resample(Date="1D").mean(dim="Date")
# combine with older data
longueuil_up2thermistor = longueuil_ext.sel(Date=slice(None, thermistor_daily.Date.isel(Date=0))).isel(Date=slice(None, -1))
...
# Longueuil.dat4835.dat to Longueuil.dat4844.dat do not have timestamps... so we add 2024-12-17 based on the creation time of the files
manu_range = np.arange(4836, 4860)
stime = time.ctime(os.path.getmtime("/storage/thermistor/Longueuil.dat" + str(manu_range[0]-1) + ".dat"))
etime = time.ctime(os.path.getmtime("/storage/thermistor/Longueuil.dat" + str(manu_range[-1]-1) + ".dat"))
starttime = datetime.datetime.strptime(stime, "%a %b %d %H:%M:%S %Y")
endtime = datetime.datetime.strptime(etime, "%a %b %d %H:%M:%S %Y")
start = str(starttime.year) + "-" + f"{starttime.month:02d}" + "-" + f"{starttime.day:02d}" + " " + f"{starttime.hour:02d}" + ":00:00"
end = str(endtime.year) + "-" + f"{endtime.month:02d}" + "-" + f"{endtime.day:02d}" + " " + f"{endtime.hour:02d}" + ":59:59"
tr = pd.date_range(start=start, end=end, freq="1min")
# function to read thermistor data
...
#
t = 0
da = read_thermistor("/storage/thermistor/Longueuil.dat" + str(manu_range[0]) + ".dat")
...
combined.to_netcdf("/storage/jrieck/SLICEop/downloads/Twater/Twater_Longueuil_permanent.nc")
# The file for the updated dataset contains the same as the permanent for now.
combined.to_netcdf("/storage/jrieck/SLICEop/downloads/Twater/Twater_Longueuil_updated.nc")
...
with open("/storage/jrieck/SLICEop/downloads/Twater/next.i", "w") as f:
    f.write("4860")
f.close()
